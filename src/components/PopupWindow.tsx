...
export default function PopupWindow({
  ...
  const accentStyle = { borderColor: colors.accent };

  return (
    <main className={`${styles.backdrop} ${!visible ? styles.hide : ""}`}>
      <div
        ...
        style={
          {
            ...
            "--transparent-accent": `${colors.accent}66`,
            ...(glowing
      ...
      >
        <header
          onClick={handleClose}
          style={{ color: colors.accent, ...accentStyle }}
        >
          <div className={styles.title}>
            <span style={{ ...accentStyle }} className={styles.icon}>
              {icon}
            </span>
            <b>{title}</b>
          </div>
        ...
        <section className={styles.content} style={{ ...accentStyle }}>
          {children}
        </section>
      </div>

      {handleClose && <small>Click the window header to dismiss.</small>}
    </main>
  );
}
